...
import { FaPaperPlane, FaKey, FaRobot } from 'react-icons/fa6';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white px-4 pt-10 pb-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <button
          ...
          className={`p-2 rounded-full ${groqApiKey ? 'text-green-500 bg-green-50' : 'text-gray-400 bg-gray-100'}`}
        >
          <FaKey size={16} />
        </button>
      </div>
      ...
      <AnimatePresence>
        {showKeyInput && (
          <motion.div
            ...
            className="overflow-hidden bg-accent2 border-b border-accent1 px-4 py-3"
          >
            <p className="text-xs text-gray-600 mb-2 font-medium">Groq API Key (BYOK)</p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white rounded-xl px-3 py-2 text-sm outline-none border border-accent1"
              />
              <button
                onClick={handleSaveKey}
                className="bg-primary text-white text-sm px-4 rounded-xl font-medium"
              >
                Simpan
              </button>
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-4 flex flex-col gap-3">
          ...
          <motion.div
            ...
            className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
              msg.role === 'user'
                ...
                : 'bg-white text-gray-800 self-start shadow-sm border border-gray-100 rounded-bl-sm'
            }`}
          >
            {msg.content}
          </motion.div>
        ...
        {loading && (
          <div className="self-start bg-white rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm border border-gray-100">
      ...
      <div className="bg-white border-t border-gray-100 px-4 py-3 pb-24 flex gap-2">
        <input
          ...
          className="flex-1 bg-neutral-bg rounded-xl px-4 py-2.5 text-sm outline-none"
        />
        <button
          id="btn-send"
          onClick={handleSend}
          disabled={loading || !input.trim()}
          className="bg-primary text-white w-10 h-10 rounded-xl flex items-center justify-center disabled:opacity-40 shrink-0"
        >
